...
単体テストは外部APIへ接続しません。収集機の再試行、絞り込み、価格対応、重複除外、11列CSV、metadata正規化、multiline、候補選別機の11列入力・14列出力、境界値、取得日時基準、URL正規化、分散選択、0件出力に加え、分析入力変換機の14列・14キー伝播、数値正規化と、分析結果契約機の14キー入力、全階層重複キー検出、ISO 8601受理形式、バージョン境界、固定キー順、原子的保存、既存出力保護、バイト単位の決定性を確認します。
